...
class EncryptInstanceDataVolumes(HuaweiCloudBaseAction):
    ...
    def process_volume(self, ecs_client, evs_client, cbr_client, instance_id, vol_set):
        ...
        instance_running = self.stop_instance(ecs_client, instance_id)
        ...
        for volume, new_volume_id in paired:
            ...
            ecs_client.attach_server_volume(request)
            # attach isn't immediately consistent
            time.sleep(self.data.get('delay', 15))
    ...
    def stop_instance(self, client, instance_id):
        ...
        instance_state = response.server.status
